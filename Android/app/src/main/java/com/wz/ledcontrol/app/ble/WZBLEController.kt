...
import com.wz.ledcontrol.app.ble.WZBluetoothUtil.getBluetoothAdapter

import com.wz.ledcontrol.app.utils.CONNECTION_MONITOR_INTERVAL
import com.wz.ledcontrol.app.utils.CONNECTION_TIMEOUT
import com.wz.ledcontrol.app.utils.DEVICE_NAME
import com.wz.ledcontrol.app.utils.LED_SERVICE_UUID
import com.wz.ledcontrol.app.utils.MAX_RETRY_COUNT
import com.wz.ledcontrol.app.utils.MTU_ACCEPTABLE
import com.wz.ledcontrol.app.utils.MTU_DEFAULT
import com.wz.ledcontrol.app.utils.MTU_FALLBACK
import com.wz.ledcontrol.app.utils.MTU_GOOD
import com.wz.ledcontrol.app.utils.MTU_OPTIMAL
import com.wz.ledcontrol.app.utils.RETRY_DELAY
import com.wz.ledcontrol.app.utils.SCAN_TIMEOUT
import com.wz.ledcontrol.app.utils.SERVICE_DISCOVERY_TIMEOUT
import com.wz.ledcontrol.app.utils.logd
import com.wz.ledcontrol.app.utils.CoroutineUtil
import com.wz.ledcontrol.app.utils.PHY_1M
import com.wz.ledcontrol.app.utils.DeviceManager
import com.wz.ledcontrol.app.utils.LED_CHARACTERISTIC_BRIGHTNESS_UUID
...
private const val SCOPE_NAME = "WZBLEController"

@SuppressLint("MissingPermission")
class WZBLEController(
    ...
    private var mBLEWZBLECallback: WZBLECallback = DefaultBLECallback()
    ...
    // PHY管理
    private val phyManager = WZPhyManager(mContext)
    ...
    private fun handleConnectionFailure(reason: String) {
        ...
        if (retryCount < MAX_RETRY_COUNT) {
            ...
            logd("重试次数已达上限，连接失败")
            retryCount = 0
            mBLEWZBLECallback.onScanFailed()
        }
    }
    ...
    private fun handleUnexpectedDisconnection() {
        ...
        mBLEWZBLECallback.onDisConnected()
    }
     ...
     * 扫描设备
     */
    fun scan(callback: WZBLECallback) {
        if (isScanning) {
            logd("扫描已在进行中，忽略重复请求")
            return
        }

        logd("=== WZBLEController: 扫描模式 ===")
        mBLEWZBLECallback = callback
        retryCount = 0

        val bluetoothAdapter = WZBluetoothUtil.getBluetoothAdapter(mContext)
     ...
     */
    fun connect(callback: WZBLECallback) {
        if (isConnecting) {
            logd("连接已在进行中，忽略重复请求")
            return
        }
     ...
     * 直接连接指定设备（通过设备地址）
     */
    fun connectToDeviceByAddress(deviceAddress: String, callback: WZBLECallback) {
        if (isConnecting) {
            logd("连接已在进行中，忽略重复请求")
            return
        }

        logd("=== WZBLEController: 直接连接模式 ===")
        mBLEWZBLECallback = callback
        retryCount = 0
    ...
    private fun connectDevice(device: BluetoothDevice) {
        ...
        mGatt = device.connectGatt(mContext, false, object : BluetoothGattCallback() {
            override fun onConnectionStateChange(gatt: BluetoothGatt?, status: Int, newState: Int) {
                ...
                when (newState) {
                    BluetoothProfile.STATE_CONNECTED -> {
                        ...
                        mBLEWZBLECallback.onConnected(device.name, device.address)
                    ...
                    BluetoothProfile.STATE_CONNECTING -> {
                        logd("正在连接设备...")
                        updateConnectionState(BLEConnectionState.CONNECTING)
                        mBLEWZBLECallback.onConnecting()
                    }
                }
            }
                ...
                characteristic?.value?.let { value ->
                    ...
                    if (characteristicUUID == LED_CHARACTERISTIC_BRIGHTNESS_UUID) {
                        try {
                            ...
                            // 通知回调处理亮度值
                            mBLEWZBLECallback.onBrightnessReceived(brightness)
            ...
            override fun onCharacteristicWrite(
                ...
                mBLEWZBLECallback.writeDataCallback(status == BluetoothGatt.GATT_SUCCESS)
            }
        ...
        service?.characteristics?.forEach { characteristic ->
            ...
            if (characUUID == mDeviceCharacteristicID) {
                ...
                if (success) {
                    ...
                    isConnecting = false
                    mBLEWZBLECallback.onCheckCharacteristicSuccess()
                } else {
                    logd("错误: 启用特征值通知失败")
    ...
    fun sendText(serviceUUID: String, characteristicUUID: String, text: String) {
        if (connectionState != BLEConnectionState.READY) {
            ...
            mBLEWZBLECallback.writeDataCallback(false)
            return
        }
    ...
    private fun sendNextChunk(
        ...
        if (currentIndex >= sendQueue.size) {
            ...
            mBLEWZBLECallback.writeDataCallback(true)
            return
        }
    ...
    private fun handleMtuNegotiationResult(gatt: BluetoothGatt?, mtu: Int, status: Int) {
        if (status == BluetoothGatt.GATT_SUCCESS) {
            ...
            // 通知回调
            mBLEWZBLECallback.onMtuNegotiationSuccess(mtu)
    ...
    private fun handleMtuNegotiationFailure(requestedMtu: Int, actualMtu: Int) {
        ...
        // 通知回调
        mBLEWZBLECallback.onMtuNegotiationFailed(requestedMtu, actualMtu)
    }
    ...
    private fun startPhyNegotiation(gatt: BluetoothGatt?) {
        ...
        // 开始PHY协商，WZPhyManager内部处理所有重试和降级逻辑
        phyManager.startPhyNegotiation(gatt, mBLEWZBLECallback)
    }
    ...
    fun readCurrentPhy() {
        if (mGatt != null) {
            logd("=== 读取当前PHY ===")
            phyManager.readPhy(mGatt, mBLEWZBLECallback)
        } else {
    ...
    private fun handleBondStateChange(device: BluetoothDevice?, bondState: Int) {
        ...
        when (bondState) {
            ...
            BluetoothDevice.BOND_BONDED -> {
                ...
                // 通知回调
                mBLEWZBLECallback.onBondingSuccess(device.name, device.address)
            }
        }
    }
