...
import com.wz.ledcontrol.app.utils.PHY_2M
import com.wz.ledcontrol.app.utils.PHY_CODED
import com.wz.ledcontrol.app.utils.PHY_NEGOTIATION_RETRY_COUNT
import com.wz.ledcontrol.app.utils.PHY_NEGOTIATION_RETRY_DELAY
import com.wz.ledcontrol.app.utils.PHY_NEGOTIATION_TIMEOUT
 ...
 */
class WZPhyManager(private val context: Context) {

    companion object {
        private const val TAG = "WZPhyManager"
    }
    ...
    // 回调接口
    private var phyCallback: WZBLECallback? = null
    ...
    fun isLe2MPhySupported(): Boolean {
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            try {
                val bluetoothAdapter = WZBluetoothUtil.getBluetoothAdapter(context)
     ...
     * 开始PHY协商
     */
    fun startPhyNegotiation(gatt: BluetoothGatt?, callback: WZBLECallback) {
        if (gatt == null) {
        ...
        // 设置PHY协商超时
        phyNegotiationTimeoutJob = CoroutineUtil.delayInScope("WZPhyManager", PHY_NEGOTIATION_TIMEOUT) {
            logd("=== PHY协商超时 ===")
     ...
     * 读取当前PHY
     */
    fun readPhy(gatt: BluetoothGatt?, callback: WZBLECallback) {
        if (gatt == null) {
            logd("GATT为空，无法读取PHY")
    ...
    private fun handlePhyNegotiationFailure(requestedPhy: Int, actualPhy: Int) {
        ...
        if (phyNegotiationRetryCount < PHY_NEGOTIATION_RETRY_COUNT) {
            ...
            phyRetryJob = CoroutineUtil.delayInScope("WZPhyManager", PHY_NEGOTIATION_RETRY_DELAY) {
                logd("开始PHY协商重试...")
                // 重试时使用降级的PHY
